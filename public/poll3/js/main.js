var pollCollectionName="Poll_3";
var pollAdminCollectionName="pollAdmin";
var pollAdminCollectionDocName="adminAccess3";
...
function addPollFirebase(question, option)
{
  ...
  var reponseArray = Array(optionCount).fill(0);
   ...
   db.collection(pollCollectionName).add({
       question: question,
       option: option,
       totalResponse: 0,
...
{
    db.collection(pollCollectionName).get().then(snapshot =>{
...
{
    db.collection(pollCollectionName).doc(pollId).delete().then(function() {
      
        // db.collection(pollResultDoc).doc(pollId).delete().then(function() {
...
publishBtn.addEventListener("click", function(){
    var adminDocRef = db.collection(pollAdminCollectionName).doc(pollAdminCollectionDocName);
...
(function(){
    db.collection(pollAdminCollectionName).doc(pollAdminCollectionDocName).get().then(doc =>{
          const data = doc.data();

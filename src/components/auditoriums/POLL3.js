...
class PublicPoll extends Component {
    ...
    suscribeToPollAdmin = () => 
    {
        ...
        this.removeListener = this.context.db.collection(AppString.PollAdmin_Col).doc(AppString.PollAdmin_Doc3).onSnapshot(
                doc => {
    ...
    getPolls = async () => {
        try {
            this.pollDocData = [];
            const snapshots = await this.context.db.collection(AppString.Poll_Doc3).get();
    ...
    sendPollFeedback = (pollId, optionNumber) => {
        var pollDocRef = this.context.db.collection(AppString.Poll_Doc3).doc(pollId);

...
class PublicPoll extends Component {
    ...
    suscribeToPollAdmin = () => 
    {
        ...
        this.removeListener = this.context.db.collection(AppString.PollAdmin_Col).doc(AppString.PollAdmin_Doc).onSnapshot(
                doc => {
    ...
    getPolls = async () => {
        try {
            this.pollDocData = [];
            const snapshots = await this.context.db.collection(AppString.Poll_Doc).get();
    ...
    sendPollFeedback = (pollId, optionNumber) => {
        var pollDocRef = this.context.db.collection(AppString.Poll_Doc).doc(pollId);

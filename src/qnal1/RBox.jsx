...
var idx=[];
var loopCount=-1;
dbx.collection(AppString.QNAL1).orderBy('time','desc').where('status', '==', '1')
    .get()
...
function useNotes(id)
{
  ...
  useEffect(() => {
    ...
    loopCount++;
    dbx.collection(AppString.QNAREPLYL1).orderBy('time','desc').where('replytoid', '==',id).onSnapshot((snapshot) => {

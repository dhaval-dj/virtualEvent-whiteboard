...
var idx=[];
var loopCount=-1;
dbx.collection(AppString.QNA).orderBy('time','desc').where('status', '==', '1')
    .get()
...
function useNotes(id)
{
  ...
  useEffect(() => {
    ...
    loopCount++;
    dbx.collection(AppString.QNAREPLY).orderBy('time','desc').where('replytoid', '==',id).onSnapshot((snapshot) => {

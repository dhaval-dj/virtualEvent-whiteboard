...
var idx=[];
var loopCount=-1;
dbx.collection(AppString.QNAL5).orderBy('time','desc').where('status', '==', '1')
    .get()
...
function useNotes(id)
{
  ...
  useEffect(() => {
    ...
    loopCount++;
    dbx.collection(AppString.QNAREPLYL5).orderBy('time','desc').where('replytoid', '==',id).onSnapshot((snapshot) => {
